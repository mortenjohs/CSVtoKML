...
require 'optparse'  # gem install optparse
...
option_parser = OptionParser.new do |opts|
  ...
  opts.on("-m MAP-NAME", "--map-name MAP-NAME", "Name of the map.\n Default: #{options[:map_name]}") do |name|
    options[:map_name] = name
  end
  opts.on("-e ENGINE-NAME", "--engine-name ENGINE-NAME", "Name of the geocoding engine eg google, yahoo.\n Default: #{options[:engine].to_s}") do |engine|
...
in_file.each do |row|
  ...
  unless coordinates_defined
    geo = Geocoder.search(row[address_column_name])
    unless geo.empty?
  ...
  folder.features << KML::Placemark.new(
    :name => row[0],  # first column is added as a name of the placemark...
